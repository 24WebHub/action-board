...
const Navigation =  () => {
  ...
  const firstname = user.fullname && user.fullname.split(' ')[0]
    ...
    return (
      ...
      <div className='lg:flex items-center hidden gap-2 font-semibold rounded border border-white py-1 px-4'>
      <BiUserCircle className='h-5 w-5' />
      <p className='capitalize'>Hi, {firstname}</p>
      </div>
      <button onClick={() => signOut()} className='flex gap-2 items-center rounded border border-white py-1 px-4'><RiLogoutCircleLine /><h3 className='font-semibold text-[13px] md:text-[16px]'>Logout</h3></button>
      </div>}
