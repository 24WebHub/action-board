...
const ToDoTasks = () => {
const {myTodoTasks, updateTaskStatus, getTaskDetails, handleTaskEdit, handleTask, deleteTask, isLoading} = useAppContext()
...
const taskEditHandler = (e, id, title, project, completionDate, status) => {
 const taskInfo = {
  id,
  title, 
  project, 
  ...
  return (
    ...
    <p className='font-motserrat text-gary-500 leading-[100%] px-2 text-end italic text-[12px]'>{task.project}</p>
    <div className='h-[1px] w-full bg-gray-100'></div> 
      ...
      <div className='space-x-2 text-[12px]'>
        <button onClick={e => taskEditHandler(e, task._id, task.title, task.project, task.completionDate, task.status)} className='ml-2 px-3 bg-green-100 rounded-sm opacity-90 text-green-900 font-semibold'>Edit</button>
